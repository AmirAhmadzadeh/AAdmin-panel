...
const auth = (props) => {
  ...
  const loginHandler = (email, pass) => dispatch(login(authState.email, authState.pass));
    ...
    <ValidatorForm onSubmit={authenticate} className="auth">
      ...
      <FormGroup>
        <FormControl className="form__controller">
          <TextValidator
            label="email"
            onChange={(e) => authDispatche({type: EMAIL_CHANGED,payload :e.target.value }) }
            name="email"
        ...
        <FormControl className="form__controller">
          ...
          <TextValidator
            label="password"
            onChange={(e) => authDispatche({ type:PASS_CHANGED , payload: e.target.value })}
            name="password"
        ...
        <FormControl className="form__controller">
          <Button type="submit" color="secondary"
            disabled={(authState.email && authState.password) ? false : true}
